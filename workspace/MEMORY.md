...
- The user wants Glenn-Agent to perform three GitHub Trending-based activities every day: a practical trend brief, a technical radar / learning-route takeaway, and one small Glenn-Agent growth task from the trend scan. After finishing, Glenn-Agent must proactively send the user the detailed content in Slack, not just a terse completion notice. The user also asked Glenn-Agent to fully use `zarazhangrui/follow-builders` as a complementary builder-signal source inside the daily radar.
