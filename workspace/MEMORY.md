...
- Kata Containers (`kata-containers/kata-containers`) is a future learning/contribution target. The repo has been forked to `glenn-agent/kata-containers` and cloned at `/workspace/openclaw/projects/kata-containers/upstream` for study. Do not contribute upstream or open PRs there until the user explicitly agrees that the timing is mature.
- The user wants Glenn-Agent to perform three GitHub Trending-based activities every day: a practical trend brief, a technical radar / learning-route takeaway, and one small Glenn-Agent growth task from the trend scan. After finishing, Glenn-Agent must proactively send the user the detailed content in Slack, not just a terse completion notice. The user also asked Glenn-Agent to fully use `zarazhangrui/follow-builders` as a complementary builder-signal source inside the daily radar. The daily radar Slack report should be Chinese-first, split into readable sections, and ideally sent as a short title message followed by threaded replies to that title for each section (`GitHub Trending`, `Builder Signal`, `技术雷达 / 学习路线`, `Glenn-Agent 成长任务`, `写回/阻塞`). Daily radar lessons should be accumulated into applied experience, not only notes; Glenn-Agent should consider building and maintaining a useful, professional open-source project where these lessons are continuously applied, tested in practice, and refined over time.
...
- The user wants Glenn-Agent to push harder for more merged upstream PRs and not stay confined to documentation. Contribution selection should use a non-documentation-first search by default: small runtime bugs, CLI behavior fixes, focused regression tests, narrow tooling fixes, and low-risk refactors should outrank docs when confidence is high, scope is clear, verification is real, and the change is not speculative. Docs remain acceptable as fallback or companion work, but future docs selections should record why no safe non-doc candidate was chosen.
