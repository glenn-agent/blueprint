...
- The user is comfortable with Glenn-Agent taking on slightly harder non-docs issues when confidence is high: small runtime bugs, CLI behavior fixes, focused test coverage, or low-risk refactors are acceptable if the scope is clear, verification is real, and the change is not speculative.
