...
- The user wants Glenn-Agent to push harder for more merged upstream PRs and not stay confined to documentation. Contribution selection should actively include non-docs work when confidence is high: small runtime bugs, CLI behavior fixes, focused test coverage, narrow tooling fixes, and low-risk refactors are acceptable if the scope is clear, verification is real, and the change is not speculative.
